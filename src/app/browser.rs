...
impl Ui {
    pub fn open_browser(
        ...
        let active_sid = tiles::active_terminal_session(&self.tile_tree);
        let active_wid = tiles::active_browser_view_id(&self.tile_tree);

        let existing_overlay = if let Some(wid) = active_wid {
            self.browser_tabs.iter().position(|wt| {
        ...
        if let Some(idx) = existing_overlay {
            ...
            self.browser_tabs[idx].browser.navigate(url);
            self.browser_tabs[idx].browser.view.set_visible(true);
        ...
        .or_else(|e| {
            ...
            Webview::new(&self.frame.window, url, content_bounds)
        });
        ...
        match wv_result {
            Ok(wv) => {
                ...
                if let Some(prev) = self.browser_tabs.get_mut(self.active_browser)
                ...
                self.browser_manager.register(id, ZLayer::Overlay);
    ...
    pub fn close_active_browser(&mut self, ctx: &mut AppCtx<'_>) {
        if self.browser_tabs.is_empty() {
            return;
        }

        let removed_id = self.browser_tabs[self.active_browser].browser.id;
        self.browser_tabs.remove(self.active_browser);
        self.browser_manager.unregister(removed_id);
        ...
        if self.browser_tabs.is_empty() {
            ...
            if self.active_browser >= self.browser_tabs.len() {
                self.active_browser = self.browser_tabs.len() - 1;
            }
    ...
    pub fn switch_browser_tab(&mut self, ctx: &mut AppCtx<'_>, index: usize) {
        if index >= self.browser_tabs.len() {
            return;
        }

        if let Some(current) = self.browser_tabs.get_mut(self.active_browser) {
        ...
        if let Some(tab) = self.browser_tabs.get_mut(index) {
            tab.browser.view.set_visible(true);

            if tab.mode == BrowserViewMode::Docked {
                tiles::activate_browser_view_tab(&mut self.tile_tree, tab.browser.id);
            }
