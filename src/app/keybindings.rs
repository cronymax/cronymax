...
pub(crate) fn handle_action(ui: &mut Ui, ctx: &mut AppCtx<'_>, action: Action) {
    match action {
        Action::NewChat => {
            let id = *ctx.next_id;
            *ctx.next_id += 1;
            ...
            let phys = ui.frame.window.inner_size();
            let logical = phys.to_logical::<f32>(ui.frame.window.scale_factor());
            ...
            tiles::add_chat_tab(&mut ui.tile_tree, id, "cronymax");
        ...
        Action::NewTerminal => {
            ...
            *ctx.next_id += 1;
            ...
            ctx.sessions.insert(id, session);
            tiles::add_terminal_tab(&mut ui.tile_tree, id, "cronymax");
        ...
        Action::CloseTab => {
            if let Some(sid) = tiles::active_terminal_session(&ui.tile_tree) {
                ctx.sessions.remove(&sid);
                tiles::remove_terminal_pane(&mut ui.tile_tree, sid);
        ...
        Action::NextTab => {
            tiles::next_terminal_tab(&mut ui.tile_tree);
            ctx.scheduler.mark_dirty();
        }
        Action::PrevTab => {
            tiles::prev_terminal_tab(&mut ui.tile_tree);
            ctx.scheduler.mark_dirty();
        }
        Action::Copy => {
            if let Some(sid) = tiles::active_terminal_session(&ui.tile_tree)
                && let Some(session) = ctx.sessions.get(&sid)
            {
                ...
                let text_buf = if let Some(sel) = &ui.terminal_selection {
        ...
        Action::Paste => {
            if let Some(text) = input::paste_from_clipboard()
                && let Some(sid) = tiles::active_terminal_session(&ui.tile_tree)
        ...
        Action::SplitVertical | Action::SplitHorizontal => {
            ...
            if let Some(active_sid) = tiles::active_terminal_session(&ui.tile_tree)
                && let Some(active_tile) = tiles::find_terminal_tile(&ui.tile_tree, active_sid)
            {
                ...
                *ctx.next_id += 1;
                let shell = ctx
                    ...
                    .unwrap_or_else(crate::renderer::platform::default_shell);
                let (_, cols, rows) = ui::compute_single_pane(
                    ui.frame.window.inner_size().width,
                    ui.frame.window.inner_size().height,
                    &ctx.cell_size,
                    &ui.styles,
                );
                let sandbox = ctx.active_sandbox_policy();
                ...
                tiles::split_pane_dir(&mut ui.tile_tree, active_tile, new_pane, dir, true);
                log::info!("Split {:?}: new session {}", dir, id);
                ctx.scheduler.mark_dirty();
            }
        }
        Action::CommandMode => {
            ui.colon_buf = Some(String::new());
...
pub(crate) fn new_terminal_with_shell(ui: &mut Ui, ctx: &mut AppCtx<'_>, shell: &str) {
    let id = *ctx.next_id;
    *ctx.next_id += 1;
    let phys = ui.frame.window.inner_size();
    let logical = phys.to_logical::<f32>(ui.frame.window.scale_factor());
    ...
    let shell_name = std::path::Path::new(shell)
        ...
        .and_then(|n| n.to_str())
        .unwrap_or(shell);
    tiles::add_terminal_tab(&mut ui.tile_tree, id, shell_name);
...
pub(crate) fn open_history_tab(ui: &mut Ui, ctx: &mut AppCtx<'_>) {
    ...
    for (i, tab) in ctx.ui_state.tabs.iter().enumerate() {
        if let TabInfo::Chat { title, session_id } = tab
            && title == "History"
        {
            tiles::activate_terminal_tab(&mut ui.tile_tree, *session_id);
    ...
    *ctx.next_id += 1;
    ...
    ctx.sessions.insert(id, session);
    tiles::add_chat_tab(&mut ui.tile_tree, id, "History");
    ...
    if let Ok(mgr) = ctx.profile_manager.lock() {
        ...
        if let Ok(entries) = std::fs::read_dir(&sessions_dir) {
            let mut session_records: Vec<(String, crate::app::session_persist::ChatSessionRecord)> =
                entries
                    .filter_map(|e| e.ok())
    ...
    if ctx.task_store.tasks.is_empty() {
        history_lines.push("*No scheduled tasks.*\n".to_string());
    } else {
    ...
    if ctx.scheduler_history_cache.is_empty() {
        history_lines.push("*No execution records.*\n".to_string());
    } else {
        for record in ctx.scheduler_history_cache.iter().take(30) {
            let status_icon = if record.status == "success" {
                "✓"
            } else {
                "✗"
            };
            let output_preview = if record.output.len() > 80 {
                format!("{}…", &record.output[..80])
            } else {
                record.output.clone()
            };
            history_lines.push(format!(
                "- [{}] task `{}` at {} ({}ms): {}\n",
                status_icon, record.task_id, record.timestamp, record.duration_ms, output_preview
            ));
        }
    }
    ...
    // Insert history as an info block in the prompt editor.
    if let Some(pe) = ctx.ui_state.prompt_editors.get_mut(&id) {
        pe.blocks.push(BlockMode::Stream {
            id: 0,
...
pub(crate) fn open_history_session(ui: &mut Ui, ctx: &mut AppCtx<'_>, uuid: &str) {
    ...
    *ctx.next_id += 1;
    ...
    tiles::add_chat_tab(&mut ui.tile_tree, id, &title);
