...
use crate::ui::{Ui, UiAction, model::AppCtx, tiles};
...
impl Ui {
    fn start_lark_console_automation(
        &mut self,
        ctx: &mut AppCtx<'_>,
        ...
        if let Ok(mut map) = ctx.pending_results.lock() {
        ...
        ctx.runtime.spawn(async move {
            ...
            let _ = proxy
                .send_event(AppEvent::OnboardingBrowserAutomationFinished { success, message });
        });
    }

    pub(crate) fn handle_ui_action_onboard(
        &mut self,
        ctx: &mut AppCtx<'_>,
        action: UiAction,
        ...
        match action {
            UiAction::OnboardingWizardStepChanged { step } => {
                ...
                if let Some(db) = &ctx.db_store
                    && let Some(ref mut wiz) = ctx.ui_state.onboarding_wizard_state
            ...
            UiAction::OnboardingAutomateLarkSetup { app_id } => {
                if app_id.trim().is_empty() {
                    if let Some(wiz) = ctx.ui_state.onboarding_wizard_state.as_mut() {
                        wiz.loading = false;
                        wiz.error = Some(
                    ...
                    self.start_lark_console_automation(ctx, event_loop, &app_id);
                }
            }
            UiAction::OnboardingWizardComplete {
                    ...
                    && app_secret_env.trim().is_empty()
                {
                    if let Some(wiz) = ctx.ui_state.onboarding_wizard_state.as_mut() {
                        wiz.loading = false;
                        wiz.error = Some(
                            "Provide an App Secret env var name, or switch to keychain storage."
                                .into(),
                        );
                    }
                    return;
                    ...
                    && secret_storage == crate::services::secret::SecretStorage::Keychain
                {
                    if let Err(e) = crate::ai::skills::credentials::credential_store(
                        ...
                        if let Some(wiz) = ctx.ui_state.onboarding_wizard_state.as_mut() {
                            wiz.loading = false;
                            wiz.error =
                                Some(format!("Failed to store app secret in keychain: {}", e));
                        }
                        return;
                    }
                ...
                ctx.ui_state.channels_ui_state =
                crate::ui::settings::channels::ChannelsSettingsState::from_claw_config_with_store(
                    ctx.config.claw.as_ref(),
                    ctx.secret_store.clone(),
                );

                // Mark wizard complete in DB and clear UI ctx.
                if let Some(db) = &ctx.db_store
                    && let Some(ref wiz) = ctx.ui_state.onboarding_wizard_state
                    && let Some(db_id) = wiz.db_id
                {
                ...
                runtime.spawn(async move {
                        ...
                        crate::channels::register_channels(&mut mgr, &claw_config, ss).await
                    {
                        ...
                        log::info!(
                            "Channel registered and WebSocket started after wizard completion"
                        );
                    }
            ...
            UiAction::OpenChannelTab {
                ...
                tiles::add_channel_tab(&mut self.tile_tree, &channel_id, &channel_name);
            ...
            UiAction::UninstallSkill(name) => {
                ...
                if let Some(sm) = ctx.skills_manager {
                    if let Err(e) = sm.uninstall(&name) {
                        ...
                        ctx.loaded_external_skills
                            .retain(|s| s.frontmatter.name != name);
                        ctx.skill_registry.remove_by_category("external");
            ...
            UiAction::ReloadSkills => {
                ...
                if let Some(sm) = ctx.skills_manager {
                    ctx.skill_registry.remove_by_category("external");
                    match sm.load_and_register(ctx.skill_registry, ctx.config) {
                        Ok(count) => {
