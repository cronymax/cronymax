> **Scope note (2025, post `agent-document-orchestration`):** the multi-agent
> portions of this change have been superseded by the document-driven
> orchestration model (Agent + Flow + Document). The `agent_graphs` /
> `agent_traces` tables and `agent.graph.*` bridge handlers described
> below are no longer the primary surface; new work happens in
> `flow/`, `document/`, and `web/flow/`. See
> [`docs/multi_agent_orchestration.md`](../../../docs/multi_agent_orchestration.md)
> §15 for the implemented YAML schemas and bridge channels.
