        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-2">
          ...
          {state.timeline.map((e) => {
            ...
            if (e.kind === "agent_status") {
              return (
                <div key={e.id} className="self-start text-xs font-mono opacity-60">
            ...
            if (e.kind === "handoff") {
              return (
                <div key={e.id} className="self-center text-xs font-mono opacity-70">
...
function RunPill({ run }: { run: { active: boolean; last_subkind: string | null } }) {
  ...
  const cls = run.active ? "bg-emerald-700/40 text-emerald-200" : "bg-card text-foreground/70";
  return <span className={`rounded px-2 py-0.5 text-xs uppercase ${cls}`}>{run.last_subkind.replace("_", " ")}</span>;
}
