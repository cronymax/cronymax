#!/usr/bin/env python3
"""Generate the new orchestration.rs that re-exports from agentgraph."""
import os

TARGET = os.path.join(os.path.dirname(__file__), "..", "src", "ai", "orchestration.rs")

CONTENT = r'''//! Multi-agent orchestration — typed state, agent nodes, routing, planning.
//!
//! Core orchestration types are defined in the `agentgraph` crate and re-exported
...
// Re-export all orchestration types from agentgraph.
pub use agentgraph::checkpoint::{CheckpointAction, CheckpointHandler};
pub use agentgraph::engine::LlmBackendFactory;
pub use agentgraph::node::AgentNode;
pub use agentgraph::routing::{
    AgentDescription, AgentRouter, LlmRouter, NextStep, OrchestrationStrategy, RuleRouter,
};
pub use agentgraph::state::{
