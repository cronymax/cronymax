...
std::string DisplayNameFor(const Tab *tab) {
  ...
  if (tab->kind() == TabKind::kWeb) {
    if (auto *wb = static_cast<WebTabBehavior *>(tab->behavior())) {
      if (!wb->current_title().empty())
        return wb->current_title();
      if (!wb->current_url().empty())
        return wb->current_url();
    }
  } else {
    if (auto *sb = static_cast<SimpleTabBehavior *>(tab->behavior())) {
      if (!sb->display_name().empty())
...
std::string TabManager::GetTabMeta(const TabId &id,
  ...
  if (const Tab *t = Get(id))
    return t->GetMeta(key);
  return {};
}

std::unique_ptr<TabBehavior>
TabManager::MakeBehavior(TabKind kind, const OpenParams &params) {
  ...
  case TabKind::kChat:
    ...
    return std::make_unique<SimpleTabBehavior>(
        ...
        params.display_name.empty() ? std::string("Chat") : params.display_name,
        resolve_url("about:blank"));
