...
void RuntimeProxy::Attach(RuntimeBridge* bridge) {
  ...
  bridge_ = bridge;
  bridge_sub_token_ = bridge_->Subscribe(
      [this](std::string payload) { OnPayload(payload); });
}
...
void RuntimeProxy::Detach() {
  ...
  for (auto& [id, entry] : pending_) {
    if (entry.sync_done) {
      ...
      *entry.sync_result = {{"kind", "err"},
                            {"error", {{"code", "detached"},
                                       {"message", "proxy detached"}}}};
...
std::string RuntimeProxy::NewCorrelationId() {
  ...
  const auto b = [&](int shift_from_hi, bool use_lo = false) -> uint8_t {
    if (!use_lo) return static_cast<uint8_t>((hi_v4 >> shift_from_hi) & 0xFF);
  ...
  return hex2(b(56)) + hex2(b(48)) + hex2(b(40)) + hex2(b(32)) +
         ...
         hex2(b(8)) + hex2(b(0)) +
         "-" +
         hex2(b(56, true)) + hex2(b(48, true)) +
         "-" +
         hex2(b(40, true)) + hex2(b(32, true)) + hex2(b(24, true)) +
         hex2(b(16, true)) + hex2(b(8, true)) + hex2(b(0, true));
}
...
bool RuntimeProxy::SendControl(nlohmann::json request,
  ...
  if (!bridge_->Invoke(envelope.dump())) {
    ...
    if (cb) cb({{"kind", "err"}, {"error", {{"code", "send_failed"},
                                            {"message", "bridge invoke failed"}}}},
               true);
    return false;
  }
  return true;
...
bool RuntimeProxy::SendControlSync(nlohmann::json request,
  ...
  const bool got =
      sync_cv->wait_for(ul, std::chrono::milliseconds(timeout_ms),
                        [&] { return *sync_done; });
  if (!got || *sync_err) return false;
  if (out_response) *out_response = std::move(*sync_res);
  return true;
}
...
void RuntimeProxy::OnPayload(const std::string& json_payload) {
  ...
  if (tag_it == msg.end() || !tag_it->is_string()) return;
...
void RuntimeProxy::HandleControlReply(const nlohmann::json& msg) {
  ...
  if (id_it == msg.end() || !id_it->is_string()) return;
  ...
  const bool is_error =
      response.value("kind", std::string{}) == "err";
