...
const char* AppEventKindToString(AppEventKind k) {
  switch (k) {
    case AppEventKind::kText:
      return "text";
    case AppEventKind::kAgentStatus:
      return "agent_status";
    case AppEventKind::kDocumentEvent:
      return "document_event";
    case AppEventKind::kReviewEvent:
      return "review_event";
    case AppEventKind::kHandoff:
      return "handoff";
    case AppEventKind::kError:
      return "error";
    case AppEventKind::kSystem:
      return "system";
    case AppEventKind::kFileEdited:
      return "file_edited";
    case AppEventKind::kGitCommitCreated:
      return "git_commit_created";
    case AppEventKind::kGitPushed:
      return "git_pushed";
  }
  return "system";
}
...
bool AppEventKindFromString(const std::string& s, AppEventKind* out) {
  if (s == "text") {
    *out = AppEventKind::kText;
    return true;
  }
  if (s == "agent_status") {
    *out = AppEventKind::kAgentStatus;
    return true;
  }
  if (s == "document_event") {
    *out = AppEventKind::kDocumentEvent;
    return true;
  }
  if (s == "review_event") {
    *out = AppEventKind::kReviewEvent;
    return true;
  }
  if (s == "handoff") {
    *out = AppEventKind::kHandoff;
    return true;
  }
  if (s == "error") {
    *out = AppEventKind::kError;
    return true;
  }
  if (s == "system") {
    *out = AppEventKind::kSystem;
    return true;
  }
  if (s == "file_edited") {
    *out = AppEventKind::kFileEdited;
    return true;
  }
  if (s == "git_commit_created") {
    *out = AppEventKind::kGitCommitCreated;
    return true;
  }
  if (s == "git_pushed") {
    *out = AppEventKind::kGitPushed;
    return true;
  }
  return false;
}
...
bool AppEvent::ParseJson(const std::string& text,
                         AppEvent* out,
                         std::string* err) {
  if (!out)
    return false;
  ...
  if (!v.contains("id") || !v["id"].is_string() || !v.contains("ts_ms") ||
      !v["ts_ms"].is_number() || !v.contains("kind") ||
      !v["kind"].is_string()) {
    if (err)
      *err = "missing id/ts_ms/kind";
    return false;
  }
  AppEventKind k;
  if (!AppEventKindFromString(v["kind"].get<std::string>(), &k)) {
    if (err)
      *err = "unknown kind: " + v["kind"].get<std::string>();
    return false;
  }
