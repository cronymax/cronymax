...
//!   * [`Orchestrator`] — drives the walk asynchronously. Persistence,
//!     capability dispatch, LLM streaming, etc. live in `cronymax`
//!     which composes this engine.
...
///
/// The trait is async via [`async_trait`] so implementations can freely
/// await futures (LLM calls, I/O, sleeps) without blocking a thread.
#[async_trait]
pub trait Step<S, E>: Send + Sync {
    ...
    async fn execute(&self, state: S) -> Result<Transition<S>, E>;
}

/// Boxed step type used inside the orchestrator's graph payload.
pub type BoxedStep<S, E> = Box<dyn Step<S, E>>;

/// Closure-based [`Step`] adapter for **synchronous** closures.
/// Convenience for tests and small inline graph definitions.
/// The sync closure is wrapped in `async move` transparently; no
/// `async_trait` boilerplate is needed at the call site.
...
/// Closure-based [`Step`] adapter for **async** closures.
/// Use when the step body itself needs to `.await` something.
///
/// ```rust,ignore
/// AsyncFnStep::new("fetch", |s| Box::pin(async move {
///     let result = do_async_work(s.value).await;
///     Ok(Transition::Terminal { state: result, reason: TerminalReason::Completed })
/// }))
/// ```
...
impl<S, E> std::fmt::Debug for AsyncFnStep<S, E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    E: Send + 'static,
{
    pub fn new(
        ...
        f: impl Fn(S) -> BoxFuture<'static, Result<Transition<S>, E>> + Send + Sync + 'static,
    ) -> Self {
        Self {
            label: label.into(),
            f: Arc::new(f),
...
#[async_trait]
impl<S, E> Step<S, E> for AsyncFnStep<S, E>
where
    S: Send + 'static,
    E: Send + 'static,
{
    ...
    E: Send + 'static,
{
    ...
    /// Drive the orchestration asynchronously.
    ///
    ...
    pub async fn run(
        ...
        mut state: S,
        limits: StepLimits,
    ) -> Result<RunOutcome<S>, RunError<E>> {
        ...
        loop {
            ...
            let transition = step.execute(state).await.map_err(|source| RunError::Step {
                node: current,
                source,
            })?;
            ...
            match transition {
                ...
                Transition::Terminal { state: s, reason } => {
                    ...
                    // More branches queued: resume next pending branch.
    ...
    fn check_edge(&self, from: NodeId, to: NodeId) -> Result<(), RunError<E>> {
        if self.graph.successors(from).any(|n| n == to) {
            Ok(())
        } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn linear_walk_runs_steps_in_order_and_terminates() {
        ...
        let out = o.run(State::new(), StepLimits::default()).await.unwrap();
    ...
    #[tokio::test]
    async fn goto_walks_through_three_nodes() {
        let mut o: Orchestrator<State, std::convert::Infallible> = Orchestrator::new();
        let next_b = late();
        let next_c = late();
    ...
    #[tokio::test]
    async fn branch_visits_all_successors_in_declared_order() {
        let mut o: Orchestrator<State, std::convert::Infallible> = Orchestrator::new();
        let left = late();
        let right = late();
    ...
    #[tokio::test]
    async fn invalid_next_node_is_rejected() {
        ...
        o.set_entry(a);
        let err = o.run(0, StepLimits::default()).await.unwrap_err();
        assert!(matches!(err, RunError::InvalidNextNode { .. }));
    }

    #[tokio::test]
    async fn step_limit_guards_infinite_loops() {
        let mut o: Orchestrator<u32, std::convert::Infallible> = Orchestrator::new();
        let me = late();
        let a = {
            let me = me.clone();
    ...
    #[tokio::test]
    async fn step_error_surfaces_with_node_context() {
    ...
    #[tokio::test]
    async fn cancelled_terminal_propagates() {
        let mut o: Orchestrator<u32, std::convert::Infallible> = Orchestrator::new();
        ...
        o.set_entry(a);
        let out = o.run(7, StepLimits::default()).await.unwrap();
        assert_eq!(out.reason, TerminalReason::Cancelled);
        assert_eq!(out.final_state, 7);
    }

    #[tokio::test]
    async fn no_entry_is_an_error() {
        let o: Orchestrator<(), std::convert::Infallible> = Orchestrator::new();
        let err = o.run((), StepLimits::default()).await.unwrap_err();
        assert!(matches!(err, RunError::NoEntryNode));
    }

    #[tokio::test]
    async fn async_fn_step_can_await_work() {
        let mut o: Orchestrator<u32, std::convert::Infallible> = Orchestrator::new();
        let a = o.add_step(AsyncFnStep::new("async_a", |s: u32| {
            Box::pin(async move {
        ...
        o.set_entry(a);
        let out = o.run(5, StepLimits::default()).await.unwrap();
        assert_eq!(out.final_state, 15);
        assert_eq!(out.reason, TerminalReason::Completed);
    }
}
